...
const FoodItems = ({ FoodData, selectedCategory, setSelectedCategory, setFoodData }: Props) => {
  ...
  return (
    <div className="p-md-4 p-3 food-card">
      <div>
        <div className="fw-bold pb-md-4 pb-3 pt-md-2 pt-1 fs-24">
          Select the Food item of given Menu
        </div>

        <div className="fw-bold pb-3 pt-2 fs-18 lh-100 ls-0">{data?.category} ({data?.FoodItems.length})</div>
            ...
            {data.FoodItems.map((item, index) => (
              <div key={index} className="py-3 d-flex align-items-start">
                ...
                <label htmlFor={`food-card-data-${index}`} className="ps-2">
                  <div className="fw-bold d-flex align-items-center fs-16 lh-100 mb-1 ls-0">
